...
import {
  ...
  Hexagon, Minus, Star, Cloud, Sun, Moon, Droplet, ArrowRight, ArrowLeft as ArrowLeftIcon, 
...
const PropertiesPanel = ({ selectedObject, canvas, onUpdate }) => {
  ...
  // Application des paddings indépendants
  const applyPadding = (side, value) => {
    ...
    // Mettre à jour la boîte de sélection native de Fabric pour qu'elle englobe le fond
  ...
  // Application dynamique des ombres
  ...
  return (
    <div className="props-content fade-in">
      ...
      {/* SECTION : TYPOGRAPHIE (Uniquement si Texte) */}
      {isText && (
        <section className="props-section">
      ...
      {/* SECTION : BOÎTE DE TEXTE (Uniquement si Texte) */}
      {isText && (
        <section className="props-section">
      ...
      {/* SECTION : APPARENCE (Formes standards, caché pour Text car géré au dessus) */}
      {!isGroup && !isText && (
        <section className="props-section">
      ...
      {/* SECTION : OMBRES PORTÉES (Shadows pour TOUT) */}
      <section className="props-section">
...
const DesignEditor = () => {
  ...
  useEffect(() => {
    if (fabric.Textbox && !fabric.Textbox.prototype.__customRenderBgSet) {
      ...
      fabric.Textbox.prototype._renderBackground = function(ctx) {
        if (!this.backgroundColor && !this.boxStroke) {
          return;
        }
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        if (isMounted) setDesignData({ maquette: resMaquette.data, version: resVersion.data });
      } catch { if (isMounted) setSaveStatus("Erreur"); }
    };
    loadData();
    ...
    const initCanvasState = async () => {
      try {
        if (designData.version.contenu?.objects?.length) {
          await new Promise(resolve => canvas.loadFromJSON(designData.version.contenu, resolve));
        } else if (designData.maquette.image_fond) {
          await new Promise(resolve => fabric.Image.fromURL(designData.maquette.image_fond, (img) => { if(img) canvas.add(img); resolve(); }));
        }
  ...
  useEffect(() => {
    ...
    const onChange = () => debouncedSave(fabricCanvas, designData.version._id);
    ...
    <>
      <div className="editor-layout">
        <header className="editor-header">
          <div className="header-left">
            <button className="btn-icon" onClick={() => navigate("/designer")} title="Retour"><ArrowLeft size={18} /></button>
            <div className="header-title">{designData?.maquette?.nom || "Projet sans nom"}</div>
            <span className="badge-status">{saveStatus === "Sauvegarde…" ? <Loader size={12} className="spin" /> : null} {saveStatus}</span>
          </div>
          <div className="header-center">
            ...
            <div className="toolbar-group zoom-group">
              <button className="btn-tool-icon" onClick={() => setZoomLevel(Math.max(ZOOM_MIN, fabricCanvas.getZoom() - ZOOM_STEP))}><ZoomOut size={16}/></button>
              <span className="zoom-val" onClick={() => setZoomLevel(1)}>{zoom}%</span>
              <button className="btn-tool-icon" onClick={() => setZoomLevel(Math.min(ZOOM_MAX, fabricCanvas.getZoom() + ZOOM_STEP))}><ZoomIn size={16}/></button>
            </div>
          </div>
          <div className="header-right"><button className="btn-primary" onClick={() => triggerSave(fabricCanvas, designData?.version?._id)}>Enregistrer</button></div>
        </header>
        ...
        <div className="editor-body">
          ...
          <main className="editor-canvas-area" onDragOver={(e) => e.preventDefault()} onDrop={handleCanvasDrop}>
            {!designData ? <div className="loader-container"><Loader size={48} className="spin text-primary" /><p>Chargement du studio...</p></div>
          ...
          <aside className="editor-right-panel">
            ...
            <div className="right-panel-body custom-scrollbar">
              {rightTab === "props" ? <PropertiesPanel selectedObject={selectedObj} canvas={fabricCanvas} onUpdate={() => { debouncedSave(fabricCanvas, designData?.version?._id); setLayersKey(k=>k+1); }} />
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* HEADER */
        .editor-header { height: 60px; background: var(--surface); display: flex; align-items: center; justify-content: space-between; padding: 0 20px; border-bottom: 1px solid var(--border); box-shadow: var(--shadow-sm); z-index: 10; }
        ...
        .zoom-val { font-size: 13px; font-weight: 600; width: 44px; text-align: center; }
        ...
        .right-tab.active::after { content: ''; position: absolute; bottom: -1px; left: 0; right: 0; height: 2px; background: var(--surface); }
        ...
        .props-toggle { flex: 1; background: var(--bg-app); border: none; border-right: 1px solid var(--border); padding: 8px 0; cursor: pointer; display: flex; justify-content: center; color: var(--text-muted); }
        ...
        .layers-order-btns button { padding: 8px; border: 1px solid var(--border); border-radius: 6px; background: var(--bg-app); cursor: pointer; color: var(--text-muted); }
        ...
        .layer-thumb { width: 20px; height: 20px; border-radius: 4px; border: 1px solid rgba(0,0,0,0.1); }
        ...
        .layer-actions button { background: white; border: 1px solid var(--border); padding: 4px; border-radius: 4px; cursor: pointer; color: var(--text-muted); }
