...
const DesignEditor = () => {
  ...
  // ── 2. INIT CANVAS ──
  useEffect(() => {
    // 🔥 Correction: Ne déclencher la création du canvas que lorsque designData.maquette._id est prêt 
    // et s'assurer de ne pas le recréer si on ne change que la "version" courante dans le state.
  ...
  const handleLoadVersion = async (version) => {
    ...
    try {
      ...
      if (contenu?.objects?.length > 0) {
        // 🔥 Correction promesses pour Fabric v6 :
        ...
        // Au lieu de .clear() qui détruit les configurations de base, on retire juste les objets.
