...
import API from '../api';
import { Type, Square, Circle, Image as ImageIcon, ArrowLeft, Loader } from 'lucide-react';

const DesignEditor = () => {
  const { id } = useParams();
  const navigate = useNavigate();
  
  // 💡 L'ASTUCE ANTI-PAGE BLANCHE : On utilise une div conteneur !
  ...
  // 2️⃣ INITIALISATION DU CANVAS SÉCURISÉE CONTRE REACT
  useEffect(() => {
    if (!designData || !wrapperRef.current) return;
    
    setSaveStatus('Initialisation de l\'éditeur...');

    // Nettoyage manuel du DOM (Empêche React de dupliquer les Canvas)
    wrapperRef.current.innerHTML = ''; 
    const canvasElement = document.createElement('canvas');
    wrapperRef.current.appendChild(canvasElement);

    // Initialisation FabricJS
    const canvas = new fabric.Canvas(canvasElement, {
      width: 900, height: 600, backgroundColor: '#f0f2f5', preserveObjectStacking: true,
    });
    setFabricCanvas(canvas);

    const { maquette, version } = designData;

    // Fonction de chargement compatible Fabric v5 ET Fabric v6
    const loadCanvasContent = async () => {
      try {
        // A. S'il y a déjà un design sauvegardé (Rect, Cercle, etc.)
        if (version.contenu && version.contenu.objects && version.contenu.objects.length > 0) {
          const loadRes = canvas.loadFromJSON(version.contenu, () => {
            canvas.renderAll(); setSaveStatus('À jour');
          });
          // Si Fabric retourne une Promise (v6+)
          if (loadRes && typeof loadRes.then === 'function') {
            await loadRes; canvas.renderAll(); setSaveStatus('À jour');
          }
        } 
        // B. S'il y a une image de fond choisie à la création
        else if (maquette.image_fond) {
          const imgRes = fabric.Image.fromURL(maquette.image_fond, (img) => {
            if (img && !imgRes) { // Version Fabric v5
              img.scaleToWidth(600); img.set({ left: 150, top: 50 });
              canvas.add(img); canvas.renderAll(); setSaveStatus('À jour');
              API.put(`/versions/${version._id}`, { contenu: canvas.toJSON() }); // Sauvegarde auto
            }
          });
          // Si Fabric retourne une Promise (v6+)
          if (imgRes && typeof imgRes.then === 'function') {
            ...
            img.scaleToWidth(600); img.set({ left: 150, top: 50 });
            canvas.add(img); canvas.renderAll(); setSaveStatus('À jour');
            API.put(`/versions/${version._id}`, { contenu: canvas.toJSON() });
          }
        } else {
  ...
  }, [designData]); // S'exécute uniquement quand on a reçu les données de MongoDB

  // 3️⃣ GESTION DE LA SAUVEGARDE AUTOMATIQUE
  ...
  const debouncedSave = useRef(debounce((c, v) => triggerSave(c, v), 1500)).current;
  ...
  const addCircle = () => {
    ...
    fabricCanvas.add(circle); fabricCanvas.setActiveObject(circle);
  };

  // ✅ CORRECTION DE L'IMPORTATION DE NOUVELLES IMAGES
  const handleImportImage = async (e) => {
    ...
    e.target.value = null; // Réinitialise l'input pour pouvoir ré-importer la même image
  };
  ...
  return (
    <>
        ...
        <div className="editor-body">
          ...
          <div className="editor-canvas-container">
            {!designData ? (
               ...
               // La div protectrice au lieu du `<canvas>` direct !
               <div ref={wrapperRef} className="canvas-shadow" style={{ width: 900, height: 600 }}></div>
            )}
          </div>
        </div>
      </div>
      ...
      <style>{`
        .editor-layout { display: flex; flex-direction: column; height: 100vh; background: #eef2f6; position: fixed; top: 0; left: 0; right: 0; z-index: 2000; }
        .editor-header { height: 60px; background: white; display: flex; align-items: center; justify-content: space-between; padding: 0 20px; box-shadow: 0 2px 5px rgba(0,0,0,0.05); }
        .btn-back { display: flex; gap: 8px; align-items: center; background: none; border: none; font-weight: 600; cursor: pointer; color: #555; }
        .save-status { display: flex; align-items: center; gap: 8px; color: #666; font-size: 14px; min-width: 150px; justify-content: flex-end; }
        .editor-body { display: flex; flex: 1; overflow: hidden; }
        .editor-toolbar { width: 80px; background: white; display: flex; flex-direction: column; align-items: center; padding: 20px 0; gap: 15px; border-right: 1px solid #ddd; overflow-y: auto; }
        .tool-btn { display: flex; flex-direction: column; align-items: center; gap: 5px; background: none; border: none; font-size: 11px; font-weight: 600; color: #555; cursor: pointer; padding: 10px; border-radius: 8px; width: 60px; transition: 0.2s; }
        ...
        .editor-canvas-container { flex: 1; display: flex; align-items: center; justify-content: center; overflow: auto; padding: 20px; }
        .canvas-shadow { box-shadow: 0 10px 30px rgba(0,0,0,0.15) !important; border-radius: 4px; overflow: hidden; background: white; }
