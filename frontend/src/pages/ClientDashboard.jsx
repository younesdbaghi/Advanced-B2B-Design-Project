    ...
    <div style={{ position:'relative', width:'100%' }}>
      {loadingCanvas && (
        <div style={{
          ...
          background:'#F8FAFC', borderRadius:10, zIndex:10, minHeight:300,
        }}>
          <Loader size={32} color="#7C3AED" style={{ marginBottom:12 }}/>
          <span style={{ color:'#64748B', fontSize:14 }}>Chargement de la maquette...</span>
        </div>
      )}
      ...
      {error ? (
        <div style={{
          display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center',
          minHeight:300, color:'#DC2626', gap:8,
        }}>
          <AlertCircle size={32}/>
...
const ClientDashboard = () => {
  ...
  const fetchProjects = async () => {
    ...
    try {
      ...
      const mesProjets = Array.isArray(data)
        ? data.filter(p => String(p.id_client?._id || p.id_client) === String(user?.id))
        : [];
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      setMsg({ type: 'error', text: `❌ ${err.response?.data?.message || 'Erreur modification'}` });
    } finally { setSending(false); }
  };
  ...
  const handleAnnuler = async (p) => {
    ...
    try {
      ...
      setMsg({ type: 'error', text: `❌ ${err.response?.data?.message || 'Erreur annulation'}` });
    }
  };
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16, marginBottom:16 }}>
              ...
              <div style={{ gridColumn:'1/-1' }}>
                ...
                <textarea value={form.description}
                  ...
                  placeholder="Décrivez votre projet..." rows={4} style={{ ...inp, resize:'vertical' }}/>
              </div>
            </div>
        ...
        <div className="card" style={{ marginBottom:24, borderLeft:'4px solid #D97706' }}>
          ...
          <div className="table-container">
            <table>
              <thead>
                <tr>
                  <th>Projet</th><th>Date début</th><th>Date fin</th><th>Statut</th>
                        ...
                        <form onSubmit={handleEditSubmit}>
                          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr 1fr', gap:12, marginBottom:12 }}>
                            ...
                            <div>
                              ...
                              <input type="date" value={editForm.date_début} onChange={e => setEditForm({...editForm, date_début:e.target.value})} required style={inp}/>
                            </div>
                            <div>
                              <label style={lbl}>Date fin *</label>
                              <input type="date" value={editForm.date_fin} onChange={e => setEditForm({...editForm, date_fin:e.target.value})} required style={inp}/>
                            </div>
                            <div>
                              <label style={lbl}>Description</label>
                          ...
                          <div style={{ display:'flex', gap:8 }}>
                            <button type="submit" disabled={sending} style={{
                              ...
                              {sending ? <Loader size={14}/> : <CheckCircle size={14}/>}
