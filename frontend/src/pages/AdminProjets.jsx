...
import API from "../api";
import {
  FolderPlus, Eye, Loader, CheckCircle,
  AlertCircle, Pencil, Trash2, X, Save, Calendar,
  AlignLeft, UserPlus, UserMinus, Users
} from 'lucide-react';
    ...
    <div ref={ref} style={{ position:'relative', display:'inline-block' }}>
      ...
      {open && (
        <div style={{ position:'absolute', right:0, top:'110%', zIndex:500, background:'white', borderRadius:12, minWidth:240, boxShadow:'0 8px 32px rgba(0,0,0,0.15)', border:'1px solid #E2E8F0', overflow:'hidden' }}>
          <div style={{ padding:'10px 14px', borderBottom:'1px solid #F1F5F9', fontSize:12, fontWeight:700, color:'#64748B', textTransform:'uppercase', letterSpacing:'0.05em' }}>
    ...
    <Modal title="Détails du projet" onClose={onClose} maxWidth={620}>
      <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:12 }}>
          <div><div style={{ fontSize:13, color:'#94A3B8', marginBottom:4 }}>Nom du projet</div><div style={{ fontSize:17, fontWeight:700 }}>{projet.nom}</div></div>
        ...
        <div style={{ borderTop:'1px solid #F1F5F9', paddingTop:16 }}>
          <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:14 }}><Users size={16} color="#2563EB"/><span style={{ fontWeight:700, fontSize:15 }}>Designers assignés ({affectations.length})</span></div>
...
const EditModal = ({ projet, users, onClose, onSaved, setMsg }) => {
  const [form, setForm] = useState({ nom:projet.nom||'', description:projet.description||'', date_début:projet.date_début?projet.date_début.slice(0,10):'', date_fin:projet.date_fin?projet.date_fin.slice(0,10):'', statut:projet.statut||'En cours', id_client:projet.id_client?._id||projet.id_client||'' });
  const [saving, setSaving] = useState(false);
  const clients = users.filter(u => u.rôle === 'client');
  const handleSave = async (e) => { e.preventDefault(); setSaving(true); try { await API.put(`/projets/${projet._id}`, form); setMsg({ type:'success', text:'✅ Projet modifié.' }); onSaved(); } catch (err) { setMsg({ type:'error', text:`❌ ${err.response?.data?.message||'Erreur'}` }); } finally { setSaving(false); } };
  return (
    <Modal title="Modifier le projet" onClose={onClose}>
      <form onSubmit={handleSave}>
        ...
        <div style={{ display:'flex', gap:10, justifyContent:'flex-end' }}>
          ...
          <button type="submit" disabled={saving} style={{ ...btnPrimary, opacity:saving?0.7:1 }}>{saving?<Loader size={15}/>:<Save size={15}/>}{saving?'...':'Enregistrer'}</button>
        </div>
      </form>
    </Modal>
  );
  ...
  <Modal title="Supprimer le projet" onClose={onClose}>
    <div style={{ textAlign:'center', padding:'8px 0 16px' }}>
      ...
      <div style={{ display:'flex', gap:10, justifyContent:'center' }}>
        ...
        <button onClick={onConfirm} disabled={deleting} style={{ ...btnDanger, opacity:deleting?0.7:1 }}>{deleting?<Loader size={15}/>:<Trash2 size={15}/>}{deleting?'Suppression...':'Supprimer'}</button>
      </div>
    </div>
  </Modal>
);
...
const AdminProjets = () => {
  ...
  const [form, setForm] = useState({ nom:'', description:'', date_début:'', date_fin:'', statut:'En cours', id_client:'', demanded:false });
  ...
  const fetchProjets = async () => { setFetching(true); try { const { data } = await API.get("/projets"); setProjets(Array.isArray(data) ? data.filter(p => !(p.demanded && p.statut==='En attente')) : []); } catch {} finally { setFetching(false); } };
  useEffect(() => { fetchProjets(); API.get("/utilisateurs").then(r => setUsers(r.data)).catch(()=>{}); }, []);
  useEffect(() => { if (!msg.text) return; const t = setTimeout(()=>setMsg({type:'',text:''}),4000); return ()=>clearTimeout(t); }, [msg]);

  const handleCreate = async (e) => {
    e.preventDefault();
    if (!form.nom||!form.id_client||!form.date_début||!form.date_fin) { setMsg({type:'error',text:'❌ Champs obligatoires manquants.'}); return; }
    setProjetLoading(true);
    try { await API.post("/projets",form); setMsg({type:'success',text:'✅ Projet créé !'}); setForm({nom:'',description:'',date_début:'',date_fin:'',statut:'En cours',id_client:'',demanded:false}); setShowForm(false); fetchProjets(); }
  ...
  const handleDelete = async () => {
    setDeleting(true);
    try { await API.delete(`/projets/${deleteProjet._id}`); setMsg({type:'success',text:'✅ Projet supprimé.'}); setDeleteProjet(null); setDetailProjet(null); fetchProjets(); }
    catch (e) { setMsg({type:'error',text:`❌ ${e.response?.data?.message||'Erreur'}`}); }
  ...
  return (
    <div>
        ...
        <div className="card" style={{ marginBottom:24, borderLeft:'4px solid #2563EB', padding:24 }}>
          ...
          <form onSubmit={handleCreate}>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16, marginBottom:16 }}>
              ...
              <div><label style={lbl}>Date fin *</label><input type="date" value={form.date_fin} onChange={e=>setForm({...form,date_fin:e.target.value})} required style={inp}/></div>
              <div><label style={lbl}>Statut</label><select value={form.statut} onChange={e=>setForm({...form,statut:e.target.value})} style={inp}>{['En cours','En révision','Validé','Refusé','Terminé'].map(s=><option key={s} value={s}>{s}</option>)}</select></div>
              <div><label style={lbl}>Demandé par client</label>
                <div style={{ display:'flex', gap:8, marginTop:8 }}>
                  {[true,false].map(val=><button key={String(val)} type="button" onClick={()=>setForm({...form,demanded:val})} style={{ flex:1, padding:'10px', border:'2px solid', borderColor:form.demanded===val?'#2563EB':'#E2E8F0', borderRadius:8, cursor:'pointer', fontWeight:600, fontSize:13, background:form.demanded===val?'rgba(37,99,235,0.08)':'white', color:form.demanded===val?'#2563EB':'#64748B' }}>{val?'✅ Oui':'❌ Non'}</button>)}
                </div>
              </div>
              <div style={{ gridColumn:'1/-1' }}><label style={lbl}>Description</label><textarea value={form.description} onChange={e=>setForm({...form,description:e.target.value})} rows={3} style={{ ...inp, resize:'vertical' }}/></div>
            </div>
      ...
      <div className="card">
        {fetching ? <div style={{ display:'flex', justifyContent:'center', padding:40 }}><Loader size={32} color="#2563EB"/></div>
        : projets.length===0 ? <div style={{ textAlign:'center', padding:48, color:'#94A3B8' }}><div style={{ fontSize:48, marginBottom:12 }}>📁</div><p style={{ fontWeight:600 }}>Aucun projet.</p></div>
        : (
          <div className="table-container">
            <table>
              <thead><tr><th>Projet</th><th>Client</th><th>Dates</th><th>Statut</th><th style={{ textAlign:'center' }}>Demanded</th><th style={{ textAlign:'center' }}>Actions</th></tr></thead>
              <tbody>
                {projets.map(p => {
