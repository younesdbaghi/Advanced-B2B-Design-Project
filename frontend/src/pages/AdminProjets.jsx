...
import API from "../api";
import {
  FolderPlus, Clock, Eye, Loader, CheckCircle,
  AlertCircle, Pencil, Trash2, X, Save, Calendar,
  AlignLeft, UserPlus, UserMinus, Users, Bell, BellOff
} from 'lucide-react';
...
const Modal = ({ title, onClose, children, maxWidth = 560 }) => (
  <div style={{
    ...
    <div style={{
      ...
      animation: 'slideUp .2s ease', maxHeight: '90vh', display: 'flex', flexDirection: 'column',
    }}>
      <div style={{
...
const AssignerDropdown = ({ projet, onAssigned }) => {
  ...
  const handleAssigner = async (designerId) => {
    setAssigning(designerId);
    try {
      await API.post('/affectations', { id_projet: projet._id, id_designer: designerId });
      setMsg('✅ Assigné !');
      setDesigners(d => d.filter(x => x._id !== designerId));
      setTimeout(() => { setOpen(false); setMsg(''); onAssigned(); }, 1200);
    } catch (err) {
  ...
  return (
    <div ref={ref} style={{ position: 'relative', display: 'inline-block' }}>
      <button onClick={handleOpen} title="Assigner un designer" style={{ ...iconBtn, color: '#059669' }}>
        <UserPlus size={15}/>
      </button>
      {open && (
        <div style={{ position: 'absolute', right: 0, top: '110%', zIndex: 200, background: 'white', borderRadius: 12, minWidth: 240, boxShadow: '0 8px 32px rgba(0,0,0,0.15)', border: '1px solid #E2E8F0', overflow: 'hidden' }}>
          <div style={{ padding: '10px 14px', borderBottom: '1px solid #F1F5F9', fontSize: 12, fontWeight: 700, color: '#64748B', textTransform: 'uppercase' }}>Designers disponibles</div>
          ...
          {loading ? (
            ...
            <div style={{ padding: '12px 14px', fontSize: 13, color: '#94A3B8', textAlign: 'center' }}>Aucun designer disponible</div>
          ) : (
            designers.map(d => (
              <div key={d._id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 14px', borderBottom: '1px solid #F8FAFC', cursor: 'pointer' }}
                onMouseEnter={e => e.currentTarget.style.background = '#F8FAFC'}
                onMouseLeave={e => e.currentTarget.style.background = 'white'}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <div style={{ width: 28, height: 28, borderRadius: '50%', background: 'linear-gradient(135deg,#059669,#0D9488)', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 11 }}>
                    {d.nom?.charAt(0)}
                  </div>
                  <div>
                ...
                <button onClick={() => handleAssigner(d._id)} disabled={assigning === d._id}
                  style={{ background: '#2563EB', color: 'white', border: 'none', borderRadius: 6, padding: '4px 10px', cursor: 'pointer', fontSize: 12, fontWeight: 600 }}>
                  {assigning === d._id ? <Loader size={11}/> : 'Assigner'}
                </button>
              </div>
            ))
          )}
...
// ── Modal Détails ────────────────────────────────────────────
const DetailModal = ({ projet, onClose, onEdit, onDelete, getStatutColor, onAssigned }) => {
  const sc = getStatutColor(projet.statut);
  const [affectations, setAffectations] = useState([]);
  const [loadingAff, setLoadingAff]     = useState(true);
  const [msgAff, setMsgAff]             = useState({ type: '', text: '' });
  ...
  const fetchAffectations = async () => {
    ...
    } catch { console.error("Erreur affectations"); }
    finally { setLoadingAff(false); }
  };

  useEffect(() => { fetchAffectations(); }, []);

  const handleRetirer = async (affId) => {
    try {
      await API.delete(`/affectations/${affId}`);
      setMsgAff({ type: 'success', text: '✅ Designer retiré.' });
      fetchAffectations();
      onAssigned();
    } catch { setMsgAff({ type: 'error', text: '❌ Erreur suppression.' }); }
  };

  const nonLus = affectations.filter(a => !a.lu).length;

  return (
    <Modal title="Détails du projet" onClose={onClose} maxWidth={640}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
          ...
          <span style={{ background: sc.bg, color: sc.color, borderRadius: 20, padding: '5px 14px', fontSize: 12, fontWeight: 700, whiteSpace: 'nowrap' }}>
            {projet.statut}
          </span>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', background: '#F8FAFC', borderRadius: 10 }}>
          <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'linear-gradient(135deg,#2563EB,#7C3AED)', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 14 }}>
            {projet.id_client?.nom?.charAt(0) || '?'}
          </div>
          <div>
            <div style={{ fontSize: 14, fontWeight: 600 }}>{projet.id_client?.nom || '—'}</div>
            <div style={{ fontSize: 12, color: '#94A3B8' }}>{projet.id_client?.email}</div>
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          <div style={dCard}>
            ...
            <div style={{ fontWeight: 600, fontSize: 14 }}>{new Date(projet.date_début).toLocaleDateString('fr-FR')}</div>
          </div>
          <div style={dCard}>
            <Calendar size={14} color="#7C3AED" style={{ marginBottom: 4 }}/>
            <div style={{ fontSize: 12, color: '#94A3B8' }}>Date de fin</div>
            <div style={{ fontWeight: 600, fontSize: 14 }}>{new Date(projet.date_fin).toLocaleDateString('fr-FR')}</div>
          </div>
        </div>
        ...
        {/* ── Section Designers assignés ── */}
        <div style={{ borderTop: '1px solid #F1F5F9', paddingTop: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <Users size={16} color="#2563EB"/>
              <span style={{ fontWeight: 700, fontSize: 15 }}>Designers assignés ({affectations.length})</span>
            </div>
            {/* Badge non lus */}
            {nonLus > 0 && (
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, background: 'rgba(245,158,11,0.12)', color: '#B45309', borderRadius: 20, padding: '3px 10px', fontSize: 12, fontWeight: 700 }}>
                <Bell size={12}/> {nonLus} non lu{nonLus > 1 ? 's' : ''}
              </span>
            )}
          </div>

          {msgAff.text && (
            <div style={{ padding: '8px 12px', borderRadius: 8, marginBottom: 12, fontSize: 13, background: msgAff.type === 'success' ? '#F0FDF4' : '#FEF2F2', color: msgAff.type === 'success' ? '#059669' : '#DC2626', border: `1px solid ${msgAff.type === 'success' ? '#BBF7D0' : '#FECACA'}` }}>
              {msgAff.text}
            </div>
          )}

          {loadingAff ? (
            <div style={{ display: 'flex', justifyContent: 'center', padding: 16 }}><Loader size={20} color="#2563EB"/></div>
          ) : affectations.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '12px 0', color: '#94A3B8', fontSize: 13 }}>Aucun designer assigné.</div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {affectations.map(a => (
                <div key={a._id} style={{
                  display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                  padding: '10px 14px', background: a.lu ? '#F8FAFC' : 'rgba(245,158,11,0.06)',
                  borderRadius: 10, border: `1px solid ${a.lu ? '#E2E8F0' : 'rgba(245,158,11,0.3)'}`,
                }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <div style={{ position: 'relative' }}>
                      <div style={{ width: 32, height: 32, borderRadius: '50%', background: 'linear-gradient(135deg,#059669,#0D9488)', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 13 }}>
                        {a.id_designer?.nom?.charAt(0) || '?'}
                      </div>
                      {/* Point indicateur lu/non lu */}
                      <div style={{ position: 'absolute', bottom: 0, right: 0, width: 10, height: 10, borderRadius: '50%', background: a.lu ? '#059669' : '#F59E0B', border: '2px solid white' }}/>
                    </div>
                    <div>
                      <div style={{ fontSize: 13, fontWeight: 700, color: '#1E293B' }}>{a.id_designer?.nom}</div>
                      <div style={{ fontSize: 11, color: '#94A3B8' }}>{a.id_designer?.email}</div>
                    </div>
                  </div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    {/* Badge lu/non lu */}
                    {a.lu ? (
                      <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, background: 'rgba(5,150,105,0.1)', color: '#059669', borderRadius: 20, padding: '3px 10px', fontSize: 11, fontWeight: 700 }}>
                        <CheckCircle size={11}/> Lu
                      </span>
                    ) : (
                      <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, background: 'rgba(245,158,11,0.12)', color: '#B45309', borderRadius: 20, padding: '3px 10px', fontSize: 11, fontWeight: 700 }}>
                        <BellOff size={11}/> Non lu
                      </span>
                    )}
                    {/* Bouton retirer */}
                    <button onClick={() => handleRetirer(a._id)} style={{ background: 'none', border: '1px solid #FECACA', borderRadius: 8, cursor: 'pointer', color: '#DC2626', padding: '4px 8px', display: 'flex', alignItems: 'center', gap: 4, fontSize: 12 }}>
...
const EditModal = ({ projet, users, onClose, onSaved, setMsg }) => {
  ...
  const handleSave = async (e) => {
    ...
    if (!form.nom || !form.date_début || !form.date_fin) {
      setMsg({ type: 'error', text: '❌ Nom, date début et date fin obligatoires.' });
      return;
    }
    setSaving(true);
    try {
      ...
      <form onSubmit={handleSave}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
          <div style={{ gridColumn: '1/-1' }}>
            ...
            <input value={form.nom} onChange={e => setForm({...form, nom: e.target.value})} required style={inp}/>
          </div>
          <div>
            <label style={lbl}>Client</label>
            <select value={form.id_client} onChange={e => setForm({...form, id_client: e.target.value})} style={inp}>
          ...
          <div>
            ...
            <select value={form.statut} onChange={e => setForm({...form, statut: e.target.value})} style={inp}>
              {['En cours','En révision','Validé','Refusé','Terminé'].map(s => <option key={s} value={s}>{s}</option>)}
            </select>
          </div>
          <div>
            <label style={lbl}>Date début *</label>
            <input type="date" value={form.date_début} onChange={e => setForm({...form, date_début: e.target.value})} required style={inp}/>
          </div>
          <div>
            <label style={lbl}>Date fin *</label>
            <input type="date" value={form.date_fin} onChange={e => setForm({...form, date_fin: e.target.value})} required style={inp}/>
          </div>
          <div style={{ gridColumn: '1/-1' }}>
...
const AdminProjets = () => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setDeleteProjet(null); setDetailProjet(null);
      fetchProjets();
    } catch (err) {
      setMsg({ type: 'error', text: `❌ ${err.response?.data?.message || 'Erreur'}` });
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        ...
        <button onClick={() => setShowForm(!showForm)} style={{ display: 'flex', alignItems: 'center', gap: 8, background: '#2563EB', color: 'white', border: 'none', borderRadius: 10, padding: '10px 18px', cursor: 'pointer', fontWeight: 600, fontSize: 14 }}>
      ...
      {showForm && (
        <div className="card" style={{ marginBottom: 24, borderLeft: '4px solid #2563EB', padding: 24 }}>
          <h3 style={{ marginBottom: 20, display: 'flex', alignItems: 'center', gap: 8 }}><FolderPlus size={18}/> Créer un projet</h3>
          <form onSubmit={handleCreateProjet}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
              ...
              <div><label style={lbl}>Date début *</label><input type="date" value={form.date_début} onChange={e => setForm({...form, date_début: e.target.value})} required style={inp}/></div>
              <div><label style={lbl}>Date fin *</label><input type="date" value={form.date_fin} onChange={e => setForm({...form, date_fin: e.target.value})} required style={inp}/></div>
              <div>
                <label style={lbl}>Statut</label>
                <select value={form.statut} onChange={e => setForm({...form, statut: e.target.value})} style={inp}>
                  {['En cours','En révision','Validé','Refusé','Terminé'].map(s => <option key={s} value={s}>{s}</option>)}
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
                  {[true, false].map(val => (
                    <button key={String(val)} type="button" onClick={() => setForm({...form, demanded: val})}
                      style={{ flex: 1, padding: '10px', border: '2px solid', borderColor: form.demanded === val ? '#2563EB' : '#E2E8F0', borderRadius: 8, cursor: 'pointer', fontWeight: 600, fontSize: 13, background: form.demanded === val ? 'rgba(37,99,235,0.08)' : 'white', color: form.demanded === val ? '#2563EB' : '#64748B' }}>
                      {val ? '✅ Oui' : '❌ Non'}
                    </button>
                  ))}
                </div>
            ...
            <button type="submit" disabled={projetLoading} style={{ display: 'flex', alignItems: 'center', gap: 8, background: projetLoading ? '#93C5FD' : '#2563EB', color: 'white', border: 'none', borderRadius: 10, padding: '12px 28px', cursor: projetLoading ? 'not-allowed' : 'pointer', fontWeight: 600 }}>
      ...
      <div className="card">
        {fetching ? (
          ...
          <div style={{ textAlign: 'center', padding: 48, color: '#94A3B8' }}>
            ...
            <p style={{ fontWeight: 600 }}>Aucun projet pour l'instant.</p>
          </div>
        ) : (
          <div className="table-container">
            <table>
              <thead>
                <tr>
                  <th>Projet</th><th>Client</th><th>Dates</th><th>Statut</th>
                ...
                {projets.map(p => {
                  ...
                  return (
                    <tr key={p._id}>
                      ...
                      <td>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          <div style={{ width: 30, height: 30, borderRadius: '50%', background: 'linear-gradient(135deg,#2563EB,#7C3AED)', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 13 }}>
                      ...
                      <td>
                        <div style={{ fontSize: 12, color: '#64748B' }}>📅 {new Date(p.date_début).toLocaleDateString('fr-FR')}</div>
                        <div style={{ fontSize: 12, color: '#64748B', marginTop: 2 }}>🏁 {new Date(p.date_fin).toLocaleDateString('fr-FR')}</div>
                      </td>
                      ...
                      <td style={{ textAlign: 'center' }}>
                        <span style={{ background: p.demanded ? 'rgba(5,150,105,0.1)' : 'rgba(100,116,139,0.1)', color: p.demanded ? '#059669' : '#64748B', borderRadius: 20, padding: '4px 12px', fontSize: 12, fontWeight: 600 }}>
                          {p.demanded ? '✅ Oui' : '❌ Non'}
                        </span>
                      </td>
                      <td style={{ textAlign: 'center' }}>
                        <div style={{ display: 'flex', gap: 6, justifyContent: 'center', alignItems: 'center' }}>
                          ...
                          <AssignerDropdown projet={p} onAssigned={fetchProjets}/>
