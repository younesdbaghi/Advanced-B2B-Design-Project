...
const envoyerEmailBienvenue = async (email, nom, motDePasse) => {
  ...
  await transporter.sendMail({
    ...
    html: `<h2>Bonjour ${nom},</h2><p>Mot de passe temporaire : <strong>${motDePasse}</strong></p>`,
  });
};
...
apiRouter.post("/auth/login", async (req, res) => {
  try {
    ...
    res.status(200).json({ token, user: { id: user._id, nom: user.nom, email: user.email, rôle: user.rôle } });
  } catch (error) {
...
apiRouter.post("/utilisateurs", verifyToken, checkRole(['admin']), async (req, res) => {
  try {
    ...
    try {
      ...
      console.log("Erreur email :", mailError.message);
      return res.status(201).json({ message: "Utilisateur créé (email non envoyé).", mot_de_passe_temp: motDePasseClair, utilisateur: { _id: u._id, nom, email, rôle } });
    }
...
// : demande AVANT /projets générique
apiRouter.post("/projets/demande", verifyToken, checkRole(['client']), async (req, res) => {
  try {
    ...
    if (!nom || !date_début || !date_fin)
      return res.status(400).json({ message: "Nom, date_début et date_fin obligatoires." });
    const admin = await Utilisateur.findOne({ rôle: "admin" });
    if (!admin) return res.status(500).json({ message: "Aucun admin trouvé." });
...
apiRouter.post("/projets", verifyToken, checkRole(['admin']), async (req, res) => {
  try {
    const projet = await new Projet({ ...req.body, id_admin_createur: req.user.id }).save();
...
// ✅ CORRIGÉ : accepter/refuser AVANT route générique :id
apiRouter.patch("/projets/:id/accepter", verifyToken, checkRole(['admin']), async (req, res) => {
  try {
    const projet = await Projet.findByIdAndUpdate(req.params.id, { statut: "En cours" }, { new: true }).populate("id_client", "nom email");
    if (!projet) return res.status(404).json({ message: "Projet introuvable." });
    res.status(200).json({ message: "Projet accepté et lancé.", projet });
  } catch (error) {
...
apiRouter.patch("/projets/:id/refuser", verifyToken, checkRole(['admin']), async (req, res) => {
  try {
    const projet = await Projet.findByIdAndUpdate(req.params.id, { statut: "Refusé" }, { new: true });
    if (!projet) return res.status(404).json({ message: "Projet introuvable." });
...
apiRouter.put("/projets/valider", verifyToken, checkRole(['client']), (req, res) => res.send("Validation projet OK"));
...
const initAdmin = async () => {
  try {
    ...
    if (!adminExists) {
      const hashPassword = await bcrypt.hash("admin", await bcrypt.genSalt(10));
      await Utilisateur.create({ nom: "admin", email: "admin", mot_de_passe: hashPassword, rôle: "admin" });
...
app.listen(PORT, () => console.log(`🚀 Serveur sur le port ${PORT}`)); "Utilisateur introuvable." });
    }
